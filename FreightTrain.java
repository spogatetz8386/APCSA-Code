...
public class FreightTrain
{
    ...
    public FreightTrain(int n)
    {
         if(n > 0){
              for(int i = 0; i < n;i++){
                   this.train.add(new Boxcar("gizmos", 5, false));
              }
         } else {
              this.train.add(new Boxcar("gizmos", 5, false));
         }
    ...
    {
         String input = c.toLowerCase();
         if(input.equals("gizmos") || input.equals("gadgets") || input.equals("widgets") || input.equals("wadgets")){
    ...
    {
        this.train.get(i).callForRepair();
    }
    ...
    public static void main(String[] args) {
        ...
        success = true;
        for (Boxcar car : trainWithFiveCars.train) {
            if (!car.getCargo().equals("gadgets"))
                success = false;
        }
        if (trainWithFiveCars.train.size() > 0 && success) {
